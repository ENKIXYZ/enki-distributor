...
export function parseBalanceMap(balances: OldFormat | NewFormat[]): MerkleDistributorInfo {
    ...
    const addresses = Object.keys(dataByAddress);

    // construct a tree
    const tree = new BalanceTree(
        addresses.map((address) => ({ account: address, amount: dataByAddress[address].amount }))
    )

    // generate claims
    const claims = addresses.reduce<{
    ...
    const tokenTotal: BigNumber = addresses.reduce<BigNumber>(
